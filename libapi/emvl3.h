...
#define L3_ERR_NO_SUPPORT_APPLICATION   (L3_ERR_BASE-20)
#define L3_ERR_STEP_END                 (L3_ERR_BASE-21)
#define L3_ERR_CASH_TRANSACTION			(L3_ERR_BASE-22)
#define L3_ERR_MANUAL_TRANSACTION       (L3_ERR_BASE-23)
#define L3_ERR_SEE_PHONE               (L3_ERR_BASE-24)
#define L3_ERR_FALLBACK_CLSS               (L3_ERR_BASE-25)
...
//Entry Mode
#define L3_ENTRY_MODE_MANUAL 			0x01
#define L3_ENTRY_MODE_MSR	  			0x02
//#define L3_ENTRY_MODE_BAR_CODE 		0x03
//#define L3_ENTRY_MODE_OCR			    0x04 //optical character recognition
#define L3_ENTRY_MODE_ICC				0x05
#define L3_ENTRY_MODE_CLSS				0x07
#define L3_ENTRY_MODE_CT_FALLBACK		0x80
//#define L3_ENTRY_MODE_CLSS_FALLBACK	0x81
...
#define L3_CFG_SUPPORT_EXTERNAL_READER		(0x0004)	///< Deprecated, Support external contactless reader
#define L3_CFG_SUPPORT_UNIONPAY_DOMESTIC	(0x0008)    ///< Support Unionpay domestic OR international(if unset/default)
...
//EMVL3 mempool size set

#define MempoolBaseSize   (1*1024)   // this is reserved for EMVL3 


/*you need caluate your contact and contactless aid number.
CTaidNumber:the number of contact aid you load with NAPI_L3LoadAIDConfig.
contactless:the number of contactless aid you load with NAPI_L3LoadAIDConfig.
CusTagNumber:the number of customtag(1F811F) you load in contact AID.
*/
#define L3_MemPool_Size(CTaidCount, CLSSaidCount, CusTagCount) \
		(  MempoolBaseSize + ((CTaidCount*44 + 9*256 + CusTagCount*28) > (CLSSaidCount * 48)\
		?(CTaidCount*44 + 9*256 + CusTagCount*28):(CLSSaidCount * 48) ))

//End of EMVL3 mempool size set

typedef enum{
	L3_EMV_INIT_APP,
	L3_EMV_READ_APP_DATA,
	L3_EMV_OFFLINE_DATA_AUTH,
	L3_EMV_PROCESSING_RESTRICTIONS,
	L3_EMV_CARDHOLDER_VERIFICATION,
	L3_EMV_TERMINAL_RISK_MANAGEMENT,
	L3_EMV_TERMINAL_ACTION_ANALYSIS,
	L3_EMV_CARD_ACTION_ANALYSIS,
}L3_EMV_STEP;
...
typedef enum{
	...
	L3_CALLBACK_GET_APDU_DATA,          ///<FUNC_GET_APDU_DATA <For history reason, ExtEmvl3 use it for PAN Confrim>
	L3_CALLBACK_LED_STATUS,				///<FUNC_LED_STATUS
...
typedef enum
{
	...
	UI_PIN_STATUS,				//Used in ExtEmvL3
} L3_UIID;
...
typedef struct{
	...
	unsigned char	rid[5];
	unsigned char	index;
	unsigned char 	pkAlgorithmIndicator;/**0x00-will check hash value
									   	   |0x01-default: will not check hash value*/
											   	
	unsigned char   hashAlgorithmIndicator;/*0x00-indicate sm algorithm
									   		|0x01-default value, not sm algorithm*/
...
typedef struct candidate_list{
	...
	unsigned char resv[3];				/* usReserve bytes */
...
*@return	L3_ERR_CANCEL, L3_ERR_TIMEOUT, L3_ERR_BYPASS, L3_ERR_REMOVE_INTERRUPT,L3_ERR_GETCLT10, L3_ERR_GETCBAD, L3_ERR_FAIL
*@note
*
*@date  	18.12.07
*@brief		create
...
*@return	L3_ERR_SUCC, L3_ERR_FAIL, L3_ERR_CANCEL, L3_ERR_TIMEOUT, L3_ERR_NO_SUPPORT_APPLICATION
*@note
*
*@date  	18.12.07
*@brief		create
...
typedef enum {
    L3_RFID_1st_ON = 0x40,       /**<the 1st indication led(blue) on*/
    L3_RFID_1st_OFF = 0x80,      /**<the 1st indication led(blue) off*/
    L3_RFID_2nd_ON = 0x04,       /**<the 2nd indication led(yellow) on*/   
    L3_RFID_2nd_OFF = 0x08,      /**<the 2nd indication led(yellow) off*/
    L3_RFID_3rd_ON = 0x10,       /**<the 3rd indication led(Green) on*/
    L3_RFID_3rd_OFF = 0x20,      /**<the 3rd indication led(Green) off*/
    L3_RFID_3rd_FLICK = 0x30,	 /**<the 3rd indication led(Green) flicker*/	
    L3_RFID_4th_ON = 0x01,       /**<the 4th indication led(red) on*/   
    L3_RFID_4th_OFF = 0x02,      /**<the 4th indication led(red) off*/
    L3_RFID_HIDE = 0x00,	     /**<hide all led*/		
} L3_LED;

/*****************************************************************************
*@fn	FUNC_LED_STATUS
*@brief [Optional] callback to Control the status of each virtual LED (ON/OFF).
*@param	[IN] status. Use '|' to control more than one LEDs. e.g. (L3_RFID_4th_ON|L3_RFID_2nd_ON) means turning red LED and  yellow LED on,  other LEDs status remain unchanged.
*@param	[IN] duration: duration time of the status, when time ends should turn off the LEDs. 
					  if the value is 0 means keep the status always/all the time before next status set in.
*@return   L3_ERR_SUCC
* Note this callback may use like this:
 FUNC_LED_STATUS(L3_RFID_1st_ON|L3_RFID_2nd_OFF|L3_RFID_3rd_OFF|L3_RFID_4th_OFF, 0);
 FUNC_LED_STATUS(L3_RFID_1st_ON|L3_RFID_2nd_ON|L3_RFID_3rd_OFF|L3_RFID_4th_OFF, 0);
 FUNC_LED_STATUS(L3_RFID_1st_ON|L3_RFID_2nd_ON|L3_RFID_3rd_ON|L3_RFID_4th_ON, 500); ---EMVCo Option 1(Visa Europe compliant)
 FUNC_LED_STATUS(L3_RFID_1st_ON|L3_RFID_2nd_ON|L3_RFID_3rd_ON|L3_RFID_4th_OFF, 0);  ---EMVCo Option 2
 FUNC_LED_STATUS(L3_RFID_1st_ON|L3_RFID_2nd_ON|L3_RFID_3rd_FLICK|L3_RFID_4th_OFF, 0);---EMVCo Option 2
 FUNC_LED_STATUS(L3_RFID_HIDE, 0); 
  
*****************************************************************************/
typedef int (*FUNC_LED_STATUS)(L3_LED status, unsigned int duration);


/*****************************************************************************
*@fn		NAPI_L3Initialize
...
*			param:mem_pool_buf,mem_pool_size only needed for MPOS.

*@param		[IN] filePath:	kernel configuration file path
*@param		[IN] config:	According to configuration bitmap definitions.
*@param		[IN] mem_pool_buf:	Mempool for Emvl3(ONLY MPOS NEEDED), you need to keep mem_pool_buf alive during EMVL3 perform transaction.
*@param		[IN] mem_pool_size:	the sizeof of Mempool.(ONLY MPOS NEEDED),you can use L3_MemPool_Size(CTaidNumber, CLSSaidNumber, CusTagNumber) to get the Recommended size.
...
int NAPI_L3Initialize(char *filePath, char *config, char* mem_pool_buf, int mem_pool_size);
...
*@param		[IN] mode: Update/Get/Remove/Flush, if Flush, will delete terminal configuration
*@return	
*@retval 	0:  successful.
*@retval 	<0: failed
*@note
...
*@param 	[IN/OUT] tlv_len:  the length of tlv string
*@param		[IN] mode: Update/Get/Remove/Flush, if Flush, will delete all of the AIDs configuration
*@return	
*@retval 	0:  successful.
*@retval 	<0: failed
*@note
...
/*****************************************************************************
*@brief		(optional)Used to change the filename you want to load or used for transaction.    
*@param		[IN] interface: Contact/Contactless
*@param		[IN] filename:   the filename you want to set.for rtos, the length of filename no longer than 12 bytes.                     
*@return	
*@retval 	0:  successful.
*@retval 	<0: failed
*@note
...
int NAPI_L3SetConfigFileName(L3_CARD_INTERFACE interface, const char * filename);
...
///////////////////////////////////////////////////////////////////////////
/*****************************************************************************
*@fn		NAPI_L3PreProcess
*@brief		Contactless transaction preprocess(Optional)
*@param		[IN] data: TLV data: such as 9F02/9F03/9C 
...
*@date  	18.12.07
*****************************************************************************/
int NAPI_L3PreProcess(char *data, int dataLen, L3_TXN_RES *res);
...
/*****************************************************************************
*@fn		NAPI_L3PerformTransaction_EMV
*@brief		This function only perform CONTACT transaction, using the same callback function as NAPI_L3PerformTransaction
*			Start from Power UP ICC card, execute up to Card Action Analysis(1st GenAC),
*           And allow to control the EMV Processing step via 'emvStep'.
*@param		[IN] data: transaction data (same as NAPI_L3PerformTransaction's data)
*@param		[IN] dataLen: the length of data
*@param		[OUT] res: the transaction result, 
				   in case of ['emvStep' = any emv step before L3_EMV_CARD_ACTION_ANALYSIS(1st GAC)]
				  'res' would be either L3_TXN_OK or L3_TXN_TERMINATE,
				   L3_TXN_OK: currently transaction is going well without any error.
				   L3_TXN_TERMINATE: means something is wrong and you cannot go on this transaction.

				   in case of ['emvStep' = L3_EMV_CARD_ACTION_ANALYSIS(1st GAC)], 
				   'res' would be 
				   L3_TXN_TERMINATE, transaction terminate
				   L3_TXN_DECLINE,  transaction decline offline
				   L3_TXN_APPROVED, transaction approve offline
				   L3_TXN_ONLINE,   transaction request online authorization
		
*@param     [IN] emvStep: EMV processing step 
*@return	description
*@retval 	error code:	level 2 or level 3 error code 
*@note  you may use this function like this:
*	step 1: 
NAPI_L3DetectCard to check if a card is inserted.
*	step 2:  NAPI_L3PerformTransaction_EMV(data ,length, res, L3_EMV_READ_APP_DATA)
*			 NAPI_L3PerformTransaction_EMV(data ,length, res, L3_EMV_CARDHOLDER_VERIFICATION)
*			 ...
*            NAPI_L3PerformTransaction_EMV(data ,length, res, L3_EMV_CARD_ACTION_ANALYSIS)
*	step 3:  NAPI_L3CompleteTransaction(data ,length, res), only if a transaction request Online Authorization
*	step 4:  NAPI_L3TerminateTransaction()
*
*@date  	18.12.07
*****************************************************************************/
int NAPI_L3PerformTransaction_EMV(char *data, int dataLen, L3_TXN_RES *res, L3_EMV_STEP emvStep);
